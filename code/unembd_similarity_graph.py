...
for i, repo in enumerate(repos):
    ...
    similarities = self_similarities(U, model_name, './cache/unembd_similarities')
    torch.cuda.empty_cache()

    counts, bins = None, None
    if similarities is None:
        counts, bins, _, _ = histogram_from_similarity_cache(
            model_name, 300, -1, 1, device=device, cache_dir_root='./cache/unembd_similarities')
    else:
        counts, bins = torch_histogram(similarities, 300, -1, 1)
    torch.cuda.empty_cache()

    percentages = (counts.float() / counts.sum().float() * 100.0).cpu()
    plt.stairs(percentages, bins, label=f"{title} ({U.shape[1]}x{U.shape[0]})", alpha=0.7)

    del U, similarities
    torch.cuda.empty_cache()

plt.title('Similarities between Unembeddings')
plt.xlabel('Cosine Similarity')
plt.ylabel('Percentage (%)')
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(save_dir, 'unembedding_similarities.png'), bbox_inches='tight')
plt.close(fig)
