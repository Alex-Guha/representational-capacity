"""
Post-hoc analysis of the first hyperparameter sweep (loss_modifier in
{40, 50, 60}, lr in {0.0025, 0.005, 0.001}). Reads `hyper_search.log` produced
by `jl_hyper_search.py` and prints best-per-(k,n) combinations, win counts,
average performance, and a pivot table.

Intended to be run from this directory:
    python analyze_hyper.py
"""
...
def analyze_hyperparameters(results):
    ...
    # Add hyperparameter information based on the order
    # For each (k, n) pair, we have 9 runs in order:
    # Loss modifier 40: lr 0.0025, 0.005, 0.001
    # Loss modifier 50: lr 0.0025, 0.005, 0.001
    # Loss modifier 60: lr 0.0025, 0.005, 0.001
    ...
    for (k, n), group in grouped:
        if len(group) >= 9:
            ...
            for i in range(9):
                loss_modifier = [40, 40, 40, 50, 50, 50, 60, 60, 60][i]
